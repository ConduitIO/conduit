...
| Chaos suite (`tests/chaos`) nightly | **live now + REQUIRED** (`.github/workflows/chaos.yml`) — nightly + every PR via the always-report/fail-closed detector (the job runs on all PRs; the SIGKILL/FIFO-ack no-infra suite executes only when a data-path tree changed, fail-closed on any diff ambiguity). `tests/chaos (race, x3)` **is** in `main`'s required branch-protection contexts (blocking) — the earlier "not yet required" note was stale. `bundle-e2e` **is now required too**. `rag-e2e` already runs on every PR (no path filter) and is one branch-protection toggle away — held only for a de-flake soak. `rag-template-e2e` IS path-filtered, so it cannot become required until it is restructured to the always-report/fail-closed pattern; a path-filtered workflow never reports on unrelated PRs and would block every merge permanently. Required contexts on `main` today: `test`, `golangci-lint`, `validate-generated-files`, `build (linux/386)`, `docker build`, `tests/chaos (race, x3)`, `bundle-e2e` |
