...
(govulncheck) is **live and blocking as a ratchet**: it fails a PR that makes a NEW vulnerability
reachable, while the 24 pre-existing findings are held in a justified baseline — see Process
maturity below.
...
| Dependency vulnerability scan (govulncheck) | **live now, blocking as a ratchet** (`.github/workflows/govulncheck.yml` + `.github/govulncheck-allowlist.txt`, added 2026-08-21). Runs on every PR, on push to `main`, and nightly (05:15) via `govulncheck -format json ./...` — reachability-aware, so it reports only vulnerabilities this codebase's call graph can actually reach, not every CVE in the dependency tree. **It fails when a called-level vulnerability appears that is not in the baseline.** The baseline holds the 24 findings reachable on `main` at introduction (20 Go standard library, cleared by a toolchain bump; 1 `golang.org/x/crypto` openpgp, unmaintained upstream; 3 `github.com/hamba/avro/v2`, unfixable — upstream archived 2026-01-18, tracked in #2817), each with a written justification beside it. Failing on ANY finding would have failed every PR for problems unrelated to its diff; report-only would have been green and therefore silent. To tighten further: clear baseline entries (the job reports which are already fixed) and add `govulncheck` to `main`'s required contexts |
