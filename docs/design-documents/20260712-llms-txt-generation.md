...
- Additive: introduces two generated files, one internal generator package, and a small `mcp.Catalog`
  refactor. No public contract changes (config schema, protocol, error codes are only *read*).
...
- **Completeness of the four sources — can a source silently under-report?** Yes, twice, and both are
  ...
  reviewer's eyeball.
- **Does the guard actually catch drift?** The guard is the *already-live*
  ...
  a diff-guard cannot catch is a change that is *invisible to the generator* — e.g. a code registered
