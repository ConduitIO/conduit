...
**Cross-connector blast radius (inherited, not introduced, by this fix — flagged per DBZ-1's PR
description).** `Persister.flushNow` (`persister.go:238-271`) flushes one shared `batch` covering
_every_ connector currently pending a write in that debounce cycle, in one transaction. Under
Approach A, a slow or stuck flush for one connector now delays not only that connector's own
durable write (as today) but also every other connector's deferred plugin-ack in the same cycle,
since acks are gated on the flush completing. This was already true of the _write_ under the
current design; A extends the same shared-batch coupling to the _ack_. Worth naming explicitly
because it means one badly-behaved connector's slow store I/O can now delay other connectors'
upstream commits, not just their local position durability. Not a blocker for A (the alternative,
B, has the same coupling and makes it synchronous instead of async-deferred, which is worse), but a
candidate follow-up if per-connector flush isolation is ever wanted.
...
- The fix PR must additionally add: a graceful-shutdown (SIGTERM) variant proving
  `StopAndWait` delivers the final deferred ack before returning, and a multi-partition test
  proving the per-partition high-water-mark tracking prevents out-of-order acks across
  interleaved flush batches.
